...
const DEFAULT_TERRAIN_ASSET_ID='2767062';
...
const viewer=new Cesium.Viewer('cesiumContainer',{
  animation:false,timeline:false,baseLayerPicker:true,geocoder:true,homeButton:false,
...
$('terrainAssetId').value=DEFAULT_TERRAIN_ASSET_ID;
$('terrainMode').value='ion';
...
function dateFromInputs(){
  ...
  if(/^\d{4}-\d{2}-\d{2}$/.test(dateValue)){const p=dateValue.split('-').map(Number);y=p[0];m=p[1];d=p[2]}
  let hh=base.getHours(),mm=base.getMinutes();
  if(/^\d{1,2}:\d{2}$/.test(timeValue)){const p=timeValue.split(':').map(Number);if(Number.isFinite(p[0])&&Number.isFinite(p[1])&&p[0]>=0&&p[0]<=23&&p[1]>=0&&p[1]<=59){hh=p[0];mm=p[1]}}
...
function sunPosition(date,latDeg,lonDeg){
  const rad=Math.PI/180,deg=180/Math.PI,start=new Date(date.getFullYear(),0,0),day=Math.floor((date-start)/86400000),hour=date.getHours()+date.getMinutes()/60;
  ...
  const tz=-date.getTimezoneOffset()/60,tst=hour*60+eq+4*lonDeg-60*tz,ha=(tst/4-180)*rad,lat=latDeg*rad;
  const cosZen=Math.sin(lat)*Math.sin(decl)+Math.cos(lat)*Math.cos(decl)*Math.cos(ha),zen=Math.acos(Math.min(1,Math.max(-1,cosZen))),elev=90-zen*deg;
...
async function loadPlateau(){
  try{
    ...
    $('plateauStatus').textContent='読込完了: '+buildPlateauUrl();toast('PLATEAU建物を読み込みました')
  }catch(e){console.error(e);$('plateauStatus').textContent='PLATEAU読込失敗: '+e.message;toast('PLATEAU読込失敗')}
}

function ensureOrtho(){if(state.orthoLayer)return;const provider=new Cesium.UrlTemplateImageryProvider({url:'https://tile.plateauview.mlit.go.jp/tiles/plateau-ortho-2023/{z}/{x}/{y}.png',maximumLevel:19});state.orthoLayer=viewer.imageryLayers.addImageryProvider(provider)}
function toggleOrtho(){if(state.orthoLayer){viewer.imageryLayers.remove(state.orthoLayer);state.orthoLayer=null;toast('PLATEAU Ortho OFF');return}ensureOrtho();toast('PLATEAU Ortho ON')}

async function applyTerrain(){
  try{
    const mode=$('terrainMode').value;
    if(mode==='ellipsoid'){clearTerrain();return}
    const token=$('cesiumToken').value.trim(),assetId=Number($('terrainAssetId').value.trim()||DEFAULT_TERRAIN_ASSET_ID);
    if(!token){toast('Cesium ion Tokenを入力してください');$('terrainStatus').textContent='Token未入力です';return}
    Cesium.Ion.defaultAccessToken=token;localStorage.setItem('CESIUM_ION_TOKEN',token);
    $('terrainStatus').textContent='Japan Regional Terrainを読み込み中...';
    const terrainProvider=await Cesium.CesiumTerrainProvider.fromIonAssetId(assetId);
    viewer.scene.terrainProvider=terrainProvider;
    viewer.scene.globe.enableLighting=false;viewer.scene.shadowMap.enabled=false;viewer.scene.globe.baseColor=Cesium.Color.WHITE;viewer.scene.globe.depthTestAgainstTerrain=true;
    ensureOrtho();
    $('terrainStatus').textContent='Japan Regional Terrain ON: Asset ID '+assetId;
    toast('Terrainを適用しました')
  }catch(e){console.error(e);$('terrainStatus').textContent='Terrain適用失敗: '+e.message;toast('Terrain適用に失敗しました')}
}
...
function saveCesiumToken(){const token=$('cesiumToken').value.trim();if(!token){toast('Tokenが空です');return}localStorage.setItem('CESIUM_ION_TOKEN',token);Cesium.Ion.defaultAccessToken=token;toast('Cesium ion Tokenを保存しました')}
...
function hav(a,b){const R=6371000,rad=Math.PI/180,dlat=(b.lat-a.lat)*rad,dlng=(b.lng-a.lng)*rad,s=Math.sin(dlat/2)**2+Math.cos(a.lat*rad)*Math.cos(b.lat*rad)*Math.sin(dlng/2)**2;return 2*R*Math.asin(Math.sqrt(s))}
...
function groundHeightAt(point){
  const carto=Cesium.Cartographic.fromDegrees(point.lng,point.lat);
  let h;
  if(viewer.scene.sampleHeightSupported){try{h=viewer.scene.sampleHeight(carto,state.routeEntities)}catch(e){h=undefined}}
  if(!Number.isFinite(h)){try{h=viewer.scene.globe.getHeight(carto)}catch(e){h=undefined}}
  return Number.isFinite(h)?h:0
}
function pedestrianOrigin(point){
  const groundH=groundHeightAt(point);
  return Cesium.Cartesian3.fromDegrees(point.lng,point.lat,groundH+1.7)
}
function sunDirectionCartesian(point,date){
  const p=sunPosition(date,point.lat,point.lng);
  if(p.elevation<=0)return null;
  const az=Cesium.Math.toRadians(p.azimuth),el=Cesium.Math.toRadians(p.elevation);
  const east=Math.sin(az)*Math.cos(el),north=Math.cos(az)*Math.cos(el),up=Math.sin(el);
  const origin=pedestrianOrigin(point),enu=Cesium.Transforms.eastNorthUpToFixedFrame(origin);
  const dir=Cesium.Matrix4.multiplyByPointAsVector(enu,new Cesium.Cartesian3(east,north,up),new Cesium.Cartesian3());
  return Cesium.Cartesian3.normalize(dir,dir)
}
async function isShaded(point){
  const dir=sunDirectionCartesian(point,state.date);
  if(!dir)return true;
  const origin=pedestrianOrigin(point),ray=new Cesium.Ray(origin,dir);
  try{
    let hit;
    if(viewer.scene.pickFromRayMostDetailed)hit=await viewer.scene.pickFromRayMostDetailed(ray,state.routeEntities);
    else if(viewer.scene.pickFromRay)hit=viewer.scene.pickFromRay(ray,state.routeEntities);
    if(!hit||!hit.position)return false;
    const dist=Cesium.Cartesian3.distance(origin,hit.position);
    return dist>1.5&&dist<500
...
function drawAnalyzed(routes){clearRoutes();const minDist=Math.min(...routes.map(r=>r.distance)),minDur=Math.min(...routes.map(r=>r.duration)),mode=$('routeMode').value;routes.forEach(r=>{const detour=(r.distance-minDist)/Math.max(minDist,1),delay=(r.duration-minDur)/Math.max(minDur,1);r.score=mode==='shade'?r.shadeRatio*.78-detour*.16-delay*.06:mode==='shortest'?r.shadeRatio*.25-detour*.55-delay*.20:r.shadeRatio*.60-detour*.28-delay*.12});routes.sort((a,b)=>b.score-a.score);state.lastRoutes=routes;routes.forEach((r,rank)=>{for(let i=0;i<r.samples.length-1;i+=4){const seg=[r.samples[i],r.samples[Math.min(i+3,r.samples.length-1)]];state.routeEntities.push(viewer.entities.add({polyline:{positions:Cesium.Cartesian3.fromDegreesArray(seg.flatMap(p=>[p.lng,p.lat])),width:rank===0?6:3,material:r.flags[i]?Cesium.Color.fromCssColorString('#0ea5e9').withAlpha(.95):Cesium.Color.fromCssColorString('#ef4444').withAlpha(.9),clampToGround:$('terrainMode').value==='ion'}}))}});viewer.flyTo(state.routeEntities,{duration:1});renderResults(routes)}
...
$('heightPreset').onchange=()=>{$('heightOffsetInput').value=$('heightPreset').value;applyHeightOffset()};
$('applyOffsetBtn').onclick=()=>applyHeightOffset();
